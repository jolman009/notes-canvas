...
describe("board-store integration flows", () => {
	...
	it("denies non-member writes with Supabase 403", async () => {
		const fetchMock = vi.fn().mockResolvedValueOnce(
			jsonResponse(403, {
				code: "42501",
				message:
					'new row violates row-level security policy for table "board_state"',
			}),
		);
		vi.stubGlobal("fetch", fetchMock);
	...
	it("denies non-member reads with Supabase 403", async () => {
		...
		await expect(
			store.getBoardSnapshot("access-token", "board-123"),
		).rejects.toThrow("Supabase request failed (403)");
	...
	it("transfers ownership through RPC", async () => {
		const fetchMock = vi
			...
			.mockResolvedValueOnce(
				jsonResponse(200, [
					{ board_id: "board-123", new_owner_user_id: "user-new-owner" },
				]),
			);
		vi.stubGlobal("fetch", fetchMock);
